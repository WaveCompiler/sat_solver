...
def visualize_1d_linear(encode, output_dir="./visualizations"):
    """
...
def visualize_2d_quadratic(encode, output_dir="./visualizations"):
    """
...
def visualize_3d_cubic(encode, output_dir="./visualizations", camera_angles=[(30, 45), (60, 120)]):
    """
...
def generate_all_visualizations(encode, output_dir="./visualizations"):
    print(f"\nGenerating 1D, 2D, and 3D tensor visualizations in '{output_dir}'...")
    visualize_1d_linear(encode, output_dir)
    visualize_2d_quadratic(encode, output_dir)
    visualize_3d_cubic(encode, output_dir)
    print("All visualizations complete!\n")

def compute_energy(spins, encode):
    bias = encode["bias"]
...
def count_unsatisfied_clauses(spins, clauses):
    ...
    for clause in clauses:
        ...
        for x_idx in clause:
            idx = abs(x_idx) - 1
            val = spin_cpu[idx]
            if (x_idx > 0 and val == 1.0) or (x_idx < 0 and val == 0.0):
...
def verify_and_print_clauses(s, clauses):
    spin_list = [int(v) for v in s.cpu().tolist()]
